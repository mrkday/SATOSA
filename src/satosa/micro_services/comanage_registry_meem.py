...
class ComanageRegistryMeem(ResponseMicroService):
    ...
    def process(self, context, data):
        ...
        if user_mfa_exempt:
            ...
            meem_reminder_url = meem_reminder_url._replace(path="registry/meem_enroller/meem_reminders/remind/{}".format(meem_enroller_id))
            meem_reminder_url = meem_reminder_url._replace(query=urlencode({'countdown': mfa_exempt_time, 'return': meem_reminder_return}))
            meem_reminder_url = urlunparse(meem_reminder_url)
            logger.debug("meem_reminder_url is {}".format(meem_reminder_url))

            return Redirect(meem_reminder_url)
        ...
        token_enrollment_return = b64encode(token_enrollment_return.encode('utf-8'), altchars='._'.encode('utf-8')).decode()
        token_enrollment_return = token_enrollment_return.replace('=', '-')

        mfa_enrollment_flow_url = "{}/return:{}".format(mfa_enrollment_flow_url, token_enrollment_return)

        logger.debug("mfa_enrollment_flow_url is {}".format(mfa_enrollment_flow_url))
